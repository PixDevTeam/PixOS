...
print_vol:
	...
	call disk_convert_l2hts			; Convert
	...
	mov si, buffer+0x2B			; Point SI at label string

	mov cx, 11				; Counter for 11 chars

	mov ah, 0Eh
						; BIOS print char function
.more_label_string:
	mov al, [si]
	int 10h
	inc si
	...
	mov si, buffer+0x36			; Point SI at filesystem string
...
os_get_file_list:
	...
	mov si, buffer		        ; ES:BX should point to our buffer
	mov bx, si
...
.show_dir_init:
	...
	mov ax, 0
	mov si, buffer		; Data reader from start of filenames
...
.to_next_root_entry:
	...
	mov di, buffer		; Point to next root dir entry
	add di, ax
...
disk_read_fat:
	...
	mov si, buffer		; Set ES:BX to point to 8K OS buffer
	mov bx, 2000h
	mov es, bx
	mov bx, si
...
disk_read_root_dir:
	...
	mov si, buffer		; Set ES:BX to point to OS buffer
	mov bx, ds
	mov es, bx
	mov bx, si
